...
import { SimpleAreaFitnessStrategy } from './SimpleAreaFitnessStrategy'
import { boxTriangles } from '../../meshes/primitives'
import { identityGenome, rotatedGenome } from './testFixtures'

const strategy = new SimpleAreaFitnessStrategy()
...
describe('SimpleAreaFitnessStrategy', () => {
  ...
  it('never flips the ranking of two orientations based on a threshold, because it has none', () => {
    ...
    expect(strategy.name).toBe('simple-area')
    expect(Object.keys(strategy)).not.toContain('config')
  })

  it('does not apply any occlusion/model-on-model penalty, unlike ProjectedAreaFitnessStrategy', () => {
    // A shelf resting over other mesh geometry should score identically to
    // the same shelf open to the bed — this strategy has no occlusion grid
    // at all, by design (that's the whole point of it being the fast option).
    const base = boxTriangles(new Vector3(-1, 0, -1), new Vector3(1, 1, 1))
    const shelfOverBase = makeMesh('shelf-over-base', [
      ...base,
      new Vector3(-1, 3, -1),
      new Vector3(1, 3, -1),
      new Vector3(1, 3, 1),
    ...
    const shelfOverAir = makeMesh('shelf-over-air', [
      ...base,
      new Vector3(9, 3, -1),
      new Vector3(11, 3, -1),
      new Vector3(11, 3, 1),
    ...
    expect(strategy.score(shelfOverBase, identityGenome())).toBeCloseTo(strategy.score(shelfOverAir, identityGenome()), 10)
  })
  ...
  describe('explain', () => {
    it('returns one contribution per triangle, matching score() as their weighted average', () => {
      ...
      expect(explanation.strategyName).toBe('simple-area')
